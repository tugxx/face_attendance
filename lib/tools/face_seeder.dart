...
class _FaceSeederState extends State<FaceSeeder> {
  ...
  Future<void> _startSeeding() async {
    ...
    try {
      ...
      final byteData = await rootBundle.load('assets/dataset_test.zip');
  ...
  Future<void> _processImageFile(File tempFile, String name) async {
    ...
    try {
      ...
      if (srcImage != null) {
        ...
        List<double>? facePixels = FaceImagePipelineNative.processFile(
          smallFile.path,
          mainFace,
        );
  ...
  Map<String, double> _validateDataQuality() {
    ...
    Map<String, double> maxInterPerPerson = {};
    for (var name in names) {
      maxInterPerPerson[name] = 0.0;
    }
    ...
    for (int i = 0; i < names.length - 1; i++) {
      for (int j = i + 1; j < names.length; j++) {
        ...
        if (sim > maxInterPerPerson[u1]!) maxInterPerPerson[u1] = sim;
        if (sim > maxInterPerPerson[u2]!) maxInterPerPerson[u2] = sim;
    ...
    double realMargin = 0.0;
    double minSoftMargin = 1.0;
    String worstPerson = "";

    // --- PHẦN BỔ SUNG ĐỂ HẾT LỖI VÀ BÁO CÁO TỔNG QUAN ---
    if (interCount > 0 && intraCount > 0) {
      AppLog.info("--------------------------------------------------");
      AppLog.info(
      ...
      realMargin = minIntra - maxInter;
      if (realMargin > 0.1) {
        AppLog.info(
          "🌟 TỔNG KẾT: Model CỰC KỲ AN TOÀN! (Hard Margin: ${realMargin.toStringAsFixed(2)})",
        );
      } else if (realMargin > 0.0) {
        AppLog.info(
          "✅ TỔNG KẾT: Model an toàn, có thể set ngưỡng phân chia. (Hard Margin: ${realMargin.toStringAsFixed(2)})",
        );
      } else {
        AppLog.warning(
          "❌ NGUY HIỂM: Có sự chồng lấn! Nguy cơ nhận nhầm. (Hard Margin: ${realMargin.toStringAsFixed(2)})",
        );
      }

      // Tính Soft Margin cá nhân tệ nhất
      avgIntraPerPerson.forEach((name, personAvgIntra) {
        double personMaxInter = maxInterPerPerson[name] ?? 0.0;
        double personMargin = personAvgIntra - personMaxInter;

        if (personMargin < minSoftMargin) {
          minSoftMargin = personMargin;
          worstPerson = name;
        }
      });

      AppLog.info(
        "=> Cá nhân tệ nhất: [$worstPerson] có Soft Margin = ${minSoftMargin.toStringAsFixed(3)}",
      );
      if (minSoftMargin > 0.1) {
        AppLog.info(
          "🌟 TỔNG KẾT: Model CỰC KỲ AN TOÀN! (Worst Soft Margin: ${minSoftMargin.toStringAsFixed(2)})",
        );
      } else if (minSoftMargin > 0.0) {
        AppLog.info(
          "✅ TỔNG KẾT: Model an toàn cho TẤT CẢ mọi người. (Worst Soft Margin: ${minSoftMargin.toStringAsFixed(2)})",
        );
      } else {
        AppLog.warning(
          "❌ NGUY HIỂM: [$worstPerson] có nguy cơ nhận nhầm! (Soft Margin: ${minSoftMargin.toStringAsFixed(2)})",
        );
      }
    }
    AppLog.info("--------------------------------------------------\n");
  ...
  Future<void> _sendToServer(
    ...
    try {
      ...
      String serverUrl = "http://192.168.1.109:5000/upload-json";
